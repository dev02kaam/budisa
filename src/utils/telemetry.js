...
function normalizePayload(body = {}) {
  ...
  const gpio = rawGpio === null || rawGpio === undefined || rawGpio === ''
    ? (isControlHeartbeat ? null : 17)
    : Number(rawGpio);
  const gpioState = rawGpioState === null || rawGpioState === undefined || rawGpioState === ''
    ? (isControlHeartbeat ? null : Number(signal === 'bascula_bajada' ? 1 : 0))
    : Number(rawGpioState);
  ...
  return {
    ...
    gps: {
      latitude: toNullableNumber(latitudeRaw),
      longitude: toNullableNumber(longitudeRaw),
